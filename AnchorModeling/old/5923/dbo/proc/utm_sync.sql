...
as begin
	...
	from
		dbo.utm as tgt
		inner join core.utm_traffic_is_paid as src on
			tgt.utm_key = src.utm_key
	where
		tgt.mt_update_dt >= @load_dt
		and not (isnull(tgt.traffic_is_paid, '||') = isnull(src.traffic_is_paid, '||'))

	execute maintenance.insertexecution
		@step = N'update traffic_is_paid',
		@execguid = @exec_guid,
		@procid = @proc_id,
		@rows = @@rowcount;
